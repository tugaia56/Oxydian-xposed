...
import static it.tugaia56.obsidian.utils.Constants.Packages.CALCULATOR;
...
public class CalculatorThemeCompiler {
    private final static String TAG = CalculatorThemeCompiler.class.getSimpleName();
    private static final String PREFIX = "ObsidianComponent";
    private static final String OVERLAY_NAME = "CA1";
    private static final String ASSET_DIR = "CA";

    public static boolean buildOverlay() throws IOException {
        preExecute();
        String source = ModuleConstants.TEMP_CACHE_DIR + "/" + CALCULATOR + "/" + OVERLAY_NAME;
        if (OverlayCompiler.createManifest(OVERLAY_NAME, CALCULATOR, source)) { postExecute(true); return true; }
        if (OverlayCompiler.runAapt(source, CALCULATOR)) { postExecute(true); return true; }
    ...
    private static void preExecute() throws IOException {
        ...
        copyAssets("CompileOnDemand/" + CALCULATOR + "/" + ASSET_DIR);
        ...
        Shell.cmd("mkdir -p " + ModuleConstants.TEMP_CACHE_DIR + "/" + CALCULATOR + "/").exec();
        Shell.cmd("mv -f \"" + ModuleConstants.DATA_DIR + "/CompileOnDemand/" + CALCULATOR + "/" + ASSET_DIR
                + "\" \"" + ModuleConstants.TEMP_CACHE_DIR + "/" + CALCULATOR + "/" + OVERLAY_NAME + "\"").exec();
