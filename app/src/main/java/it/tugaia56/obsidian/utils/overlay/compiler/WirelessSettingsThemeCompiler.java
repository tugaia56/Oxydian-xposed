...
import static it.tugaia56.obsidian.utils.Constants.Packages.WIRELESS_SETTINGS;
...
public class WirelessSettingsThemeCompiler {
    private final static String TAG = WirelessSettingsThemeCompiler.class.getSimpleName();
    private static final String PREFIX = "ObsidianComponent";
    private static final String OVERLAY_NAME = "WS1";
    private static final String ASSET_DIR = "WS";

    public static boolean buildOverlay() throws IOException {
        preExecute();
        String source = ModuleConstants.TEMP_CACHE_DIR + "/" + WIRELESS_SETTINGS + "/" + OVERLAY_NAME;
        if (OverlayCompiler.createManifest(OVERLAY_NAME, WIRELESS_SETTINGS, source)) { postExecute(true); return true; }
        if (OverlayCompiler.runAapt(source, WIRELESS_SETTINGS)) { postExecute(true); return true; }
    ...
    private static void preExecute() throws IOException {
        ...
        Shell.cmd("mkdir -p " + ModuleConstants.TEMP_CACHE_DIR + "/" + WIRELESS_SETTINGS + "/").exec();
        Shell.cmd("mv -f \"" + ModuleConstants.DATA_DIR + "/CompileOnDemand/" + WIRELESS_SETTINGS + "/" + ASSET_DIR
                + "\" \"" + ModuleConstants.TEMP_CACHE_DIR + "/" + WIRELESS_SETTINGS + "/" + OVERLAY_NAME + "\"").exec();
