...
import static it.tugaia56.obsidian.utils.Constants.Packages.GALLERY;
...
public class GalleryThemeCompiler {
    private final static String TAG = GalleryThemeCompiler.class.getSimpleName();
    private static final String PREFIX = "ObsidianComponent";
    private static final String OVERLAY_NAME = "GL1";
    private static final String ASSET_DIR = "GL";

    public static boolean buildOverlay() throws IOException {
        preExecute();
        String source = ModuleConstants.TEMP_CACHE_DIR + "/" + GALLERY + "/" + OVERLAY_NAME;
        if (OverlayCompiler.createManifest(OVERLAY_NAME, GALLERY, source)) { postExecute(true); return true; }
        if (OverlayCompiler.runAapt(source, GALLERY)) { postExecute(true); return true; }
    ...
    private static void preExecute() throws IOException {
        ...
        copyAssets("CompileOnDemand/" + GALLERY + "/" + ASSET_DIR);
        ...
        Shell.cmd("mkdir -p " + ModuleConstants.TEMP_CACHE_DIR + "/" + GALLERY + "/").exec();
        Shell.cmd("mv -f \"" + ModuleConstants.DATA_DIR + "/CompileOnDemand/" + GALLERY + "/" + ASSET_DIR
                + "\" \"" + ModuleConstants.TEMP_CACHE_DIR + "/" + GALLERY + "/" + OVERLAY_NAME + "\"").exec();
