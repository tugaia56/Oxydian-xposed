...
import static it.tugaia56.obsidian.utils.Constants.Packages.CAMERA;
...
public class CameraThemeCompiler {
    private final static String TAG = CameraThemeCompiler.class.getSimpleName();
    private static final String PREFIX = "ObsidianComponent";
    private static final String OVERLAY_NAME = "CM1";
    private static final String ASSET_DIR = "CM";

    public static boolean buildOverlay() throws IOException {
        preExecute();
        String source = ModuleConstants.TEMP_CACHE_DIR + "/" + CAMERA + "/" + OVERLAY_NAME;
        if (OverlayCompiler.createManifest(OVERLAY_NAME, CAMERA, source)) { postExecute(true); return true; }
        if (OverlayCompiler.runAapt(source, CAMERA)) { postExecute(true); return true; }
    ...
    private static void preExecute() throws IOException {
        ...
        copyAssets("CompileOnDemand/" + CAMERA + "/" + ASSET_DIR);
        ...
        Shell.cmd("mkdir -p " + ModuleConstants.TEMP_CACHE_DIR + "/" + CAMERA + "/").exec();
        Shell.cmd("mv -f \"" + ModuleConstants.DATA_DIR + "/CompileOnDemand/" + CAMERA + "/" + ASSET_DIR
                + "\" \"" + ModuleConstants.TEMP_CACHE_DIR + "/" + CAMERA + "/" + OVERLAY_NAME + "\"").exec();
