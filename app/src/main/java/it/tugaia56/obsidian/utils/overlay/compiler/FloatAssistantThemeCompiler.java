...
import static it.tugaia56.obsidian.utils.Constants.Packages.FLOAT_ASSISTANT;
...
public class FloatAssistantThemeCompiler {
    private final static String TAG = FloatAssistantThemeCompiler.class.getSimpleName();
    private static final String PREFIX = "ObsidianComponent";
    private static final String OVERLAY_NAME = "FA1";
    private static final String ASSET_DIR = "FA";

    public static boolean buildOverlay() throws IOException {
        preExecute();
        String source = ModuleConstants.TEMP_CACHE_DIR + "/" + FLOAT_ASSISTANT + "/" + OVERLAY_NAME;
        if (OverlayCompiler.createManifest(OVERLAY_NAME, FLOAT_ASSISTANT, source)) { postExecute(true); return true; }
        if (OverlayCompiler.runAapt(source, FLOAT_ASSISTANT)) { postExecute(true); return true; }
    ...
    private static void preExecute() throws IOException {
        ...
        copyAssets("CompileOnDemand/" + FLOAT_ASSISTANT + "/" + ASSET_DIR);
        ...
        Shell.cmd("mkdir -p " + ModuleConstants.TEMP_CACHE_DIR + "/" + FLOAT_ASSISTANT + "/").exec();
        Shell.cmd("mv -f \"" + ModuleConstants.DATA_DIR + "/CompileOnDemand/" + FLOAT_ASSISTANT + "/" + ASSET_DIR
                + "\" \"" + ModuleConstants.TEMP_CACHE_DIR + "/" + FLOAT_ASSISTANT + "/" + OVERLAY_NAME + "\"").exec();
