...
import static it.tugaia56.obsidian.utils.Constants.Packages.OSHARE;
...
public class OShareThemeCompiler {
    private final static String TAG = OShareThemeCompiler.class.getSimpleName();
    private static final String PREFIX = "ObsidianComponent";
    private static final String OVERLAY_NAME = "OS1";
    private static final String ASSET_DIR = "OS";

    public static boolean buildOverlay() throws IOException {
        preExecute();
        String source = ModuleConstants.TEMP_CACHE_DIR + "/" + OSHARE + "/" + OVERLAY_NAME;
        if (OverlayCompiler.createManifest(OVERLAY_NAME, OSHARE, source)) { postExecute(true); return true; }
        if (OverlayCompiler.runAapt(source, OSHARE)) { postExecute(true); return true; }
    ...
    private static void preExecute() throws IOException {
        ...
        copyAssets("CompileOnDemand/" + OSHARE + "/" + ASSET_DIR);
        ...
        Shell.cmd("mkdir -p " + ModuleConstants.TEMP_CACHE_DIR + "/" + OSHARE + "/").exec();
        Shell.cmd("mv -f \"" + ModuleConstants.DATA_DIR + "/CompileOnDemand/" + OSHARE + "/" + ASSET_DIR
                + "\" \"" + ModuleConstants.TEMP_CACHE_DIR + "/" + OSHARE + "/" + OVERLAY_NAME + "\"").exec();
