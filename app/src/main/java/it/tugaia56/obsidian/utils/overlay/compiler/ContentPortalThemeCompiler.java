...
import static it.tugaia56.obsidian.utils.Constants.Packages.CONTENT_PORTAL;
...
public class ContentPortalThemeCompiler {
    private final static String TAG = ContentPortalThemeCompiler.class.getSimpleName();
    private static final String PREFIX = "ObsidianComponent";
    private static final String OVERLAY_NAME = "CP1";
    private static final String ASSET_DIR = "CP";

    public static boolean buildOverlay() throws IOException {
        preExecute();
        String source = ModuleConstants.TEMP_CACHE_DIR + "/" + CONTENT_PORTAL + "/" + OVERLAY_NAME;
        if (OverlayCompiler.createManifest(OVERLAY_NAME, CONTENT_PORTAL, source)) { postExecute(true); return true; }
        if (OverlayCompiler.runAapt(source, CONTENT_PORTAL)) { postExecute(true); return true; }
    ...
    private static void preExecute() throws IOException {
        ...
        copyAssets("CompileOnDemand/" + CONTENT_PORTAL + "/" + ASSET_DIR);
        ...
        Shell.cmd("mkdir -p " + ModuleConstants.TEMP_CACHE_DIR + "/" + CONTENT_PORTAL + "/").exec();
        Shell.cmd("mv -f \"" + ModuleConstants.DATA_DIR + "/CompileOnDemand/" + CONTENT_PORTAL + "/" + ASSET_DIR
                + "\" \"" + ModuleConstants.TEMP_CACHE_DIR + "/" + CONTENT_PORTAL + "/" + OVERLAY_NAME + "\"").exec();
