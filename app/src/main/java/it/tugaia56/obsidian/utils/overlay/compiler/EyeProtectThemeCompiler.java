...
import static it.tugaia56.obsidian.utils.Constants.Packages.EYE_PROTECT;
...
public class EyeProtectThemeCompiler {
    private final static String TAG = EyeProtectThemeCompiler.class.getSimpleName();
    private static final String PREFIX = "ObsidianComponent";
    private static final String OVERLAY_NAME = "EP1";
    private static final String ASSET_DIR = "EP";

    public static boolean buildOverlay() throws IOException {
        preExecute();
        String source = ModuleConstants.TEMP_CACHE_DIR + "/" + EYE_PROTECT + "/" + OVERLAY_NAME;
        if (OverlayCompiler.createManifest(OVERLAY_NAME, EYE_PROTECT, source)) { postExecute(true); return true; }
        if (OverlayCompiler.runAapt(source, EYE_PROTECT)) { postExecute(true); return true; }
    ...
    private static void preExecute() throws IOException {
        ...
        copyAssets("CompileOnDemand/" + EYE_PROTECT + "/" + ASSET_DIR);
        ...
        Shell.cmd("mkdir -p " + ModuleConstants.TEMP_CACHE_DIR + "/" + EYE_PROTECT + "/").exec();
        Shell.cmd("mv -f \"" + ModuleConstants.DATA_DIR + "/CompileOnDemand/" + EYE_PROTECT + "/" + ASSET_DIR
                + "\" \"" + ModuleConstants.TEMP_CACHE_DIR + "/" + EYE_PROTECT + "/" + OVERLAY_NAME + "\"").exec();
