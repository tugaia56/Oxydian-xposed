...
import static it.tugaia56.obsidian.utils.Constants.Packages.BROWSER;
...
public class BrowserThemeCompiler {
    private final static String TAG = BrowserThemeCompiler.class.getSimpleName();
    private static final String PREFIX = "ObsidianComponent";
    private static final String OVERLAY_NAME = "BR1";
    private static final String ASSET_DIR = "BR";

    public static boolean buildOverlay() throws IOException {
        preExecute();
        String source = ModuleConstants.TEMP_CACHE_DIR + "/" + BROWSER + "/" + OVERLAY_NAME;
        if (OverlayCompiler.createManifest(OVERLAY_NAME, BROWSER, source)) { postExecute(true); return true; }
        if (OverlayCompiler.runAapt(source, BROWSER)) { postExecute(true); return true; }
    ...
    private static void preExecute() throws IOException {
        ...
        copyAssets("CompileOnDemand/" + BROWSER + "/" + ASSET_DIR);
        ...
        Shell.cmd("mkdir -p " + ModuleConstants.TEMP_CACHE_DIR + "/" + BROWSER + "/").exec();
        Shell.cmd("mv -f \"" + ModuleConstants.DATA_DIR + "/CompileOnDemand/" + BROWSER + "/" + ASSET_DIR
                + "\" \"" + ModuleConstants.TEMP_CACHE_DIR + "/" + BROWSER + "/" + OVERLAY_NAME + "\"").exec();
