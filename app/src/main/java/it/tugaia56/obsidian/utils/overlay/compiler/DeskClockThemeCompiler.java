...
import static it.tugaia56.obsidian.utils.Constants.Packages.DESK_CLOCK;
...
public class DeskClockThemeCompiler {
    private final static String TAG = DeskClockThemeCompiler.class.getSimpleName();
    private static final String PREFIX = "ObsidianComponent";
    private static final String OVERLAY_NAME = "DC1";
    private static final String ASSET_DIR = "DC";

    public static boolean buildOverlay() throws IOException {
        preExecute();
        String source = ModuleConstants.TEMP_CACHE_DIR + "/" + DESK_CLOCK + "/" + OVERLAY_NAME;
        if (OverlayCompiler.createManifest(OVERLAY_NAME, DESK_CLOCK, source)) { postExecute(true); return true; }
        if (OverlayCompiler.runAapt(source, DESK_CLOCK)) { postExecute(true); return true; }
    ...
    private static void preExecute() throws IOException {
        ...
        copyAssets("CompileOnDemand/" + DESK_CLOCK + "/" + ASSET_DIR);
        ...
        Shell.cmd("mkdir -p " + ModuleConstants.TEMP_CACHE_DIR + "/" + DESK_CLOCK + "/").exec();
        Shell.cmd("mv -f \"" + ModuleConstants.DATA_DIR + "/CompileOnDemand/" + DESK_CLOCK + "/" + ASSET_DIR
                + "\" \"" + ModuleConstants.TEMP_CACHE_DIR + "/" + DESK_CLOCK + "/" + OVERLAY_NAME + "\"").exec();
